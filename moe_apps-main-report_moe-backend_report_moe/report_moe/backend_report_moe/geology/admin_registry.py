...
from map_layers.water_feature_admin import field_schema_for_layer, list_display_for_layer
...
GROUP_ORDER = ('Master data', 'Daily operations', 'Planning', 'Monitoring')


def ordered_registry_groups() -> list[dict[str, Any]]:
    ...
    ordered = [groups[name] for name in GROUP_ORDER if name in groups]
    ordered.extend(g for name, g in groups.items() if name not in GROUP_ORDER)
    return ordered
