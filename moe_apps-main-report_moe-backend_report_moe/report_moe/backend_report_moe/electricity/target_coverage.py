...
from .models import OperationalTarget, PowerPlant
...
PLANT_DAILY_METRIC_KEYS: tuple[str, ...] = ('generation_mwh',)
...
def _required_slots(on_date: date) -> list[TargetCoverageSlot]:
    ...
    for plant in PowerPlant.objects.order_by('name_en'):
        for metric_key in PLANT_DAILY_METRIC_KEYS:
            slots.append(
                TargetCoverageSlot(
                    metric_key=metric_key,
...
def _slot_payload(slot: TargetCoverageSlot, on_date: date) -> dict[str, Any]:
    ...
    if slot.group == 'monthly_plan':
        ...
        row = resolve_target(slot.metric_key, on_date, scope_type=slot.scope_type, scope_code=slot.scope_code)
    scope_label_en = slot.scope_code or 'National'
    scope_label_ar = 'وطني'
    if slot.scope_type == OperationalTarget.ScopeType.PLANT and slot.scope_code:
        plant = PowerPlant.objects.filter(code=slot.scope_code).first()
        if plant:
            scope_label_en = plant.name_en
            scope_label_ar = plant.name_ar
...
def build_target_coverage(on_date: date) -> dict[str, Any]:
    ...
    groups_meta = {
        ...
        'plant_targets': {
            'label_en': 'Plant generation targets',
            'label_ar': 'أهداف توليد المحطات',
            'description_en': 'Required for the plant targets table (one per plant)',
            'description_ar': 'مطلوبة لجدول أهداف المحطات (هدف لكل محطة)',
        },
    }
