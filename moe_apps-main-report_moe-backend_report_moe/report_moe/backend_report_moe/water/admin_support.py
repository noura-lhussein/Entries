...
from config.serialize import apply_model_update, model_to_admin_dict
...
def _parse_clear(value) -> bool:
    if isinstance(value, bool):
        return value
    return str(value or '').lower() in ('1', 'true', 'yes', 'on')


def _action_result(result: dict) -> JsonResponse:
    return JsonResponse(result, status=200 if result.get('ok') else 400)


def _uploaded_list(files) -> list:
    if files is None:
        return []
...
def build_admin_list_payload(request, spec) -> dict[str, Any]:
    ...
    fk_names = [f.name for f in spec.model._meta.fields if isinstance(f, ForeignKey)]
    if fk_names:
    ...
    results = [model_to_admin_dict(obj) for obj in page_qs]
    return {
        'slug': spec.slug,
        'count': len(results),
