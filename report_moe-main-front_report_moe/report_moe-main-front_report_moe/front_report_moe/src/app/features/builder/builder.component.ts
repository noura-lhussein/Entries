...
export class BuilderComponent implements OnInit {
  ...
  attrForm = { id: 0, label: '', type: 'text' as Attribute['type'], required: false };
  ...
  openAttribute(attr: Attribute | null): void {
    ...
    if (attr) {
      this.attrForm = { id: attr.id, label: attr.label, type: attr.type, required: attr.required };
      if (attr.type === 'select') {
        this.loadAttributeOptions(attr);
      }
    } else {
      this.attrForm = { id: 0, label: '', type: 'text', required: false };
    }
