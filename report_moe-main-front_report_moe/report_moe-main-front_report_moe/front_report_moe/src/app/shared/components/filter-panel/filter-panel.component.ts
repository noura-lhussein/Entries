...
export class FilterPanelComponent implements OnChanges, OnDestroy {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    const skipped = !groupsChanged && !filtersChanged;
    debugLog(
      'filter-panel.component.ts:ngOnChanges',
      skipped ? 'skipped redundant' : 'input change',
      {
        activeFilters: !!changes['activeFilters'],
        filterGroups: !!changes['filterGroups'],
        groupCount: this.filterGroups.length,
        panelOpen: this.showFilters(),
        skipped,
      },
      'H4',
      'post-fix-v2',
  ...
  openFilters(): void {
    ...
    const stats = this.optionStats();
    debugLog(
      'filter-panel.component.ts:toggleFilters',
      'panel opened',
      { ms: Math.round(performance.now() - t0), ...stats },
      'H10',
      'post-fix-v2',
    );
  ...
  private optionStats(): {
    totalOptions: number;
    maxFieldOptions: number;
    truncatedFields: number;
  } {
    let totalOptions = 0;
    let maxFieldOptions = 0;
    let truncatedFields = 0;
    for (const f of this.filterGroups) {
      const raw = this.rawSelectOptions(f).length;
      const shown = this.selectOptions(f).length;
      totalOptions += shown;
      if (shown > maxFieldOptions) maxFieldOptions = shown;
      if (raw > shown) truncatedFields++;
    }
    return { totalOptions, maxFieldOptions, truncatedFields };
  }
  ...
  onSelectChange(key: string, value: string): void {
    ...
    debugLog(
      'filter-panel.component.ts:onSelectChange',
      'emit filterChange',
      { key, valueLen: String(value ?? '').length },
      'H2',
      'post-fix-v2',
    );
